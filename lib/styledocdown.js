...
function generateMD (list, options) {
     ...
     for (i = 0, len = list.length; i < len; i += 1) {
        section = [];
        description = list[i].description ? fileExists(path.join(options.root, list[i].description)): false;
        example = list[i].example ? fileExists(path.join(options.root, list[i].example)) : false;
